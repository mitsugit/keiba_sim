...
def margin_fix_jockey(self):
    ...
    place_one_target_df = target_df[(target_df['place_number'] == 1)].loc[:,
                          ["race_place", "race_number", "win_ninki", "from_odds", "win_return"]]
    ...
    for i in NewRaceID_list:
        # print("レース番号",i)
        target_place_ninki_list.append(target_df[
                                           (target_df.NewRaceID == i) & (target_df.place_number <= target_place) & (
                                                       target_df.place_number >= 1)].sort_values(
            'place_number').win_ninki.values.tolist()[0:len(range(target_place))])

    # ターゲット人気のオッズリストを作成
    for i in NewRaceID_list:
        if i in target_ninki_NewRaceID_list:

            target_ninki_odds.append(
                target_df[(target_df.NewRaceID == i) & (target_df.win_ninki == target_ninki)].from_odds.values.tolist()[
                    0])
    ...
    for i in NewRaceID_list:
        if i in target_jockey_NewRaceID_list:
            ...
            target_jockey_odds.append(
                target_df[(target_df.NewRaceID == i) & (target_df.jockey == target_jockey)].from_odds.values.tolist()[
                    0])
    ...
    for i in NewRaceID_list:
        if i in target_jockey_NewRaceID_list:
            ...
            target_jockey_ninki.append(
                target_df[(target_df.NewRaceID == i) & (target_df.jockey == target_jockey)].win_ninki.values.tolist()[
                    0])
    ...
    for i in NewRaceID_list:
        ...
        # 　リセットされた後を想定
        elif bet == initial_bet and onbet >= 1 and bet_reset >= 1:
            ...
            print("margin計算が適用")
            print(margin, "×", "連続ベット合計", total_bet, "÷", "(", float(target_ninki_odds[i - 1]), "-", margin, ")", "÷",
                  100, "×", 100)
